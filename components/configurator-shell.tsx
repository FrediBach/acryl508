...
import { ArtDesigner } from "./art-designer";
import { allSheetMaterials } from "@/lib/sheet-materials";
import { createArt, normalizeArtConfiguration, artExport, artSvg, artBuildNotes } from "@/lib/art";
...
import { speakerFabrication, artFabrication, caseFabrication, panelFabrication, protectorFabrication, standFabrication } from "@/lib/fabrication";
...
export function ConfiguratorShell() {
  ...
  const { case: config, stand: standConfig, panel: panelConfig, protector: protectorConfig, art: artConfig, speaker: speakerConfig } = history.present;
  ...
  return <div className="app-shell" onPointerDownCapture={event => {
    ...
    <dialog ref={dialog} className="info-dialog" aria-labelledby="dialog-title" onClose={() => setInfo(null)} onClick={event => { if (event.target === event.currentTarget) dialog.current?.close(); }}>
      ...
      {info === "materials" ? <><Layers3 size={29} className="dialog-icon" /><h2 id="dialog-title">Material is the design.</h2><p>Cast acrylic, also known as GS. Exposed edges and visible connections, with individual materials and thicknesses available for each sheet in case, speaker, art, protector and stand designs.</p><div className="material-library"><ColorChooser tint={mode === "speaker" ? speakerConfig.tint : mode === "art" ? artConfig.tint : mode === "panel" ? panelConfig.tint : mode === "protector" ? protectorConfig.tint : mode === "stand" ? standConfig.tint : config.tint} onChange={tint => { if (mode === "speaker") setSpeakerConfig(current => ({ ...current, ...allSheetMaterials(current, { tint }) })); else if (mode === "art") setArtConfig(current => ({ ...current, ...allSheetMaterials(current, { tint }) })); else if (mode === "panel") setPanelConfig(current => ({ ...current, tint })); else if (mode === "protector") setProtectorConfig(current => ({ ...current, ...allSheetMaterials(current, { tint }) })); else if (mode === "stand") setStandConfig(current => ({ ...current, ...allSheetMaterials(current, { tint }) })); else updateConfig({ tint, ...(config.individualPanelTints ? { panelTints: panelTintsFrom(tint) } : {}) }); }} label={mode === "speaker" ? "Speaker acrylic color" : mode === "art" ? "Art acrylic color" : mode === "panel" ? "Panel acrylic color" : mode === "protector" ? "Protector acrylic color" : mode === "stand" ? "Stand acrylic color" : "Apply color to all sheets"} /><TransparencyChooser value={mode === "speaker" ? speakerConfig.transparency : mode === "art" ? artConfig.transparency : mode === "panel" ? panelConfig.transparency : mode === "protector" ? protectorConfig.transparency : mode === "stand" ? standConfig.transparency : config.transparency} onChange={transparency => { if (mode === "speaker") setSpeakerConfig(current => ({ ...current, ...allSheetMaterials(current, { transparency }) })); else if (mode === "art") setArtConfig(current => ({ ...current, ...allSheetMaterials(current, { transparency }) })); else if (mode === "panel") setPanelConfig(current => ({ ...current, transparency })); else if (mode === "protector") setProtectorConfig(current => ({ ...current, ...allSheetMaterials(current, { transparency }) })); else if (mode === "stand") setStandConfig(current => ({ ...current, ...allSheetMaterials(current, { transparency }) })); else updateConfig({ transparency, ...(config.individualPanelTints ? { panelTransparencies: panelTransparenciesFrom(transparency) } : {}) }); }} /><MaterialPreviewNote /></div><p className="dialog-small">Colours are visual approximations. Select the actual sheets and check physical samples before fabrication. Extruded (XT) acrylic is outside this design.</p></> : mode === "speaker" ? <><h2 id="dialog-title">The same MYND. A new enclosure.</h2><p>Seven acrylic sheets around the original drivers, passive radiators and electronics.</p><ol className="build-notes">{speakerBuildNotes.map((note, index) => <li key={note}><strong>{["Keep the complete donor.", "Build a sealed chamber.", "Adapt the driver seats.", "Fit the electronics.", "Tune the prototype."][index]}</strong><span>{note}</span></li>)}</ol><p className="dialog-small">Adapted from <a href="https://github.com/teufelaudio/mynd-hardware" target="_blank" rel="noreferrer">Teufel MYND hardware</a>, CC BY-SA 4.0. Changes: flat acrylic shell, simplified apertures and dot grille.</p></> : mode === "art" ? <><h2 id="dialog-title">A little acrylic botany.</h2><p>Generative leaves on an interlocking grid, with outward bends and individual sheet controls.</p><ol className="build-notes">{artBuildNotes.map((note, index) => <li key={note}><strong>{["Assemble the roots.", "Grow the leaves.", "Form the bends.", "Dial in the fit.", "Test your sculpture."][index]}</strong><span>{note}</span></li>)}</ol></> : mode === "panel" ? <><h2 id="dialog-title">A face for your next idea.</h2><p>Blank panels, DIY openings and engraved artwork in one flat acrylic sheet.</p><ol className="build-notes">{panelBuildNotes.map((note, index) => <li key={note}><strong>{["Choose the format.", "Fit your components.", "Cut or engrave.", "Arrange the layout.", "Export the operations."][index]}</strong><span>{note}</span></li>)}</ol></> : mode === "protector" ? <><h2 id="dialog-title">A little room above the controls.</h2><p>One oversized top sheet. Adjustable edge supports and optional through-tabs with removable retaining strips.</p><ol className="build-notes">{protectorBuildNotes.map((note, index) => <li key={note}><strong>{["Locate the feet.", "Set the clearance.", "Retain the feet.", "Slot together.", "Test the prototype."][index]}</strong><span>{note}</span></li>)}</ol></> : mode === "stand" ? <><h2 id="dialog-title">Slot together. Play at your angle.</h2><p>A synth stand made entirely from flat, laser-cut acrylic. Solid support ribs and three cross braces form a slotted grid.</p><ol className="build-notes">{standBuildNotes.map((note, index) => <li key={note}><strong>{["Assemble the grid.", "Materials for each sheet.", "Support across the width.", "Dial in the fit.", "Test the prototype."][index]}</strong><span>{note}</span></li>)}</ol><p className="dialog-small">Fabrication reference: <a href="https://www.acrylite.co/resources/fabrication-manuals/laser-machining-acrylite" target="_blank" rel="noreferrer">ACRYLITE laser machining guidance</a>. Laser settings and internal corners affect stress in the cut acrylic.</p></> : <><h2 id="dialog-title">Simple parts. Thoughtful details.</h2><p>Shape your system around the way you patch. This configurator explores a removable, mechanically assembled GS acrylic enclosure.</p><ol className="build-notes"><li><strong>Materials for each sheet.</strong><span>Five enclosure panels. The side panels also form the stance and optional handles with rounded hand openings. Use one material throughout or choose a color, transparency and thickness for each sheet. Interlocking slots and tabs follow the adjoining sheet thicknesses. The preview includes aluminium rails and black hardware.</span></li><li><strong>Interlock, then secure.</strong><span>Slide the base and front/rear panel tabs into the closed slots of one side panel. Fit the rails, then the second side over the remaining tabs. The rail-end screws retain the sides with load-spreading washers. No glue or extra case-panel screws. To disassemble, support the case, remove the rail screws on one side, and withdraw that side panel.</span></li><li><strong>Find your footing.</strong><span>Choose a flat, 10°, 20° or 30° stance. Flat cases can use integral pads, arches or continuous runners, with adjustable foot height. Each additional row can add its own tilt toward the rear, up to 75° total from the table. Angled rows expand rail spacing at each bend, reshape the side and rear panels, and automatically include integral support feet. Wedge, arch and sled profiles shape the side panels themselves, keeping their contact edges level on the table. Handles extend from the left, right or both side panels; Auto uses a pair above 84 HP or from 6U. Adjust handle width and height; rounded roots blend the grips into the sides. Sled openings use thicker runners and rounded inner corners. Enable the patch cable holder to extend the rear plate with evenly spaced, rounded fingers. Adjust finger height and slit width to suit your cables. Add a patch cable board to either or both side panels: a grid of 4 mm round holes for parking patch plugs, with adjustable width, height and spacing. A handle on the same side sits above the grid. The case stays at five acrylic sheets. Grip strength and loaded stability need prototype validation.</span></li><li><strong>Space to breathe.</strong><span>Choose slits or holes, then layer waves, ripples or organic variation onto their sizes and positions. The pattern editor preserves borders and material between vents, and omits vents near custom cuts. Geometry limits still need prototype validation. The base sits above retaining material, and the side panels extend beyond the end-panel slots. Outer dimensions include these margins; your HP, row spacing and depth above the base stay unchanged.</span></li><li><strong>Power is a considered choice.</strong><span>Sinusoda Juice uses the data-sheet envelope of 226 × 86 × 19 mm and 23 headers. Its 28 mounting holes are estimated from the photograph; verify centres and diameters against your board before drilling. Use at least 14 evenly distributed screws with nylon washers. The preview assumes 5 mm standoffs; check module, cable and electrical clearances. Trolley Bus has 28 horizontal headers, a 423 × 80 mm board and a conservative 435 mm installation envelope from the setup drawing. Its eight photo-estimated screw mounts adapt Befaco’s adhesive mounting method; the two cover screws are excluded. Verify dimensions, fasteners and insulation against your board. CompactPWR uses the manufacturer’s 174 × 79 × 20 mm envelope, 20 headers and four photo-estimated corner mounts. Its separate barrel/switch or USB-C inlet and cabling are not reserved.</span></li><li><strong>Make it yours.</strong><span>Import a filled SVG or add text using a supplied or imported font. Choose an enclosure side, width, rotation and position for each cutout. The outside view shows the finished panel. After all cuts, only the largest piece attached to the original panel perimeter is kept; loose letter centres and other islands are removed, with a warning.</span></li><li><strong>A specification to build on.</strong><span>Export JSON for the complete specification, or download one full-size SVG containing every acrylic sheet as a named vector group. Dimensions describe the enclosure, excluding the integral grip, patch cable board, cable holder, feet and stance extensions. Joint fit, laser kerf, corner relief, screw engagement and loaded retention need prototype testing before fabrication.</span></li></ol></>}
    </dialog>
  </div>;
}
