  ...
  return <dialog ref={dialogRef} id="fabrication-dialog" className="fabrication-dialog" aria-labelledby="fabrication-title" aria-describedby="fabrication-description" onClick={event => { if (event.target === event.currentTarget) dialogRef.current?.close(); }}>
    ...
    <div className="fabrication-content">
      <div><h2>Parts & hardware</h2><div className="parts-table-wrap"><table className="parts-table"><thead><tr><th>Part</th><th>Size (mm)</th><th>Material</th></tr></thead><tbody>{fabrication.parts.map(part => { const bounds = polygonBounds(part.polygons); return <tr key={part.id}><td>{part.label}</td><td>{bounds.width.toFixed(1)} × {bounds.height.toFixed(1)}</td><td>{part.material} · {part.thickness ?? fabrication.thickness} mm</td></tr>; })}</tbody></table></div>
        ...
        <h3>Test the slot fit</h3><p className="control-note">Cut a comb and tab from the same measured stock thickness. Download a sample for each thickness used here. Slot widths are labelled in an engraving group; insert the tab edge-on. The sample compares 0–0.4 mm clearance{fabrication.clearance ? `, including your ${fabrication.clearance} mm setting` : ""}.</p>
        {thicknesses.map(thickness => <button key={thickness} className="cutout-button" onClick={() => downloadFile(fitCoupon(thickness, fabrication.clearance).svg, "image/svg+xml", `acryl508-${thickness}mm-fit-coupon.svg`)}>Download {thicknesses.length > 1 ? `${thickness} mm ` : ""}fit coupon</button>)}
      </div>
      <div><h2>Arrange on stock sheets</h2><p className="control-note">A simple shelf arrangement, separated by material and thickness. Dimensions and gaps are in millimetres. Parts may rotate 90°; no shapes are scaled. This is not optimized nesting.</p>
        ...
        <div className="stock-sheets">{layout.sheets.map((sheet, index) => <div className="stock-sheet" key={index}>
          ...
          <p>{index + 1}. {sheet.material} · {sheet.thickness ?? fabrication.thickness} mm · {sheet.parts.length} parts</p><button className="cutout-button" disabled={blocked} onClick={() => downloadFile(stockSvg(sheet, width, height, fabrication.warnings), "image/svg+xml", `acryl508-stock-sheet-${index + 1}.svg`)}>Download sheet {index + 1}</button>
        </div>)}</div>
      </div>
    </div>
    </div>
