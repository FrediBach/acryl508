...
export function StandObjectControls({ model, error, onChange, mode = "stand" }: { mode?: "stand" | "protector"; model?: StandObject; error?: string; onChange: (object?: StandObject) => void }) {
  ...
  return <div className="stand-object-controls">
    ...
    <p className="control-note">Use your synth or any object. Models stay in this browser session. Choose the source units, set which side is up, then adjust the object angle.</p>
    {model && <><div className="stand-object-name"><strong>{model.name}</strong><button className="icon-button" aria-label="Remove 3D model" title="Use manual dimensions" onClick={() => { ++request.current; setBusy(false); setUploadError(""); onChange(); }}><X size={15} /></button></div>
      ...
      <p className="control-note">{mode === "protector" ? "The cover clears the highest model point. Each foot follows the upper surface with a broad outside locating lip. Inspect the contacts to keep feet off delicate controls." : "Dimensions come from the model. Supports follow its underside across the whole acrylic thickness. Thin tips are automatically trimmed into blunt ends. Enable Rounded edges to soften the outline corners. Broad front lips follow the model to retain it. Their tops are capped to avoid sharp fins."}</p>
    </>}
