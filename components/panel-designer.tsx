...
export function PanelDesigner({ panel, dark, onChange, onExportJson, onExportSvg }: Props) {
  ...
  return <div className="configurator-grid panel-designer"><div className="preview-column">
    ...
    <ConfigSection number="07" title="Fabrication & export" summary="Full-size SVG · Cut + engrave · JSON"><p className="control-note">Red outlines cut through; blue filled outlines engrave the surface. Assign both groups in your laser software. Curves are sampled; check the vectors and apply kerf compensation once in CAM.</p><p className="control-note">JSON includes settings, source artwork and resolved geometry. Make a fit sample before cutting the finished panel.</p>{!panel.canExport && <p className="cutout-warning" role="alert">SVG export is unavailable until the geometry errors are resolved.</p>}<div className="cutout-actions"><button className="cutout-button" onClick={onExportJson}>Download JSON</button><button className="cutout-button" disabled={!panel.canExport} onClick={onExportSvg}>Download SVG layers</button></div></ConfigSection>
  </aside></div>;
}
