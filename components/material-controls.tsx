...
export function ColorChooser({ tint, onChange, label = "Acrylic color" }: { tint: AcrylicTint; onChange: (tint: AcrylicTint) => void; label?: string }) {
  return <><div className="field-heading"><span>{label}</span><span className="field-note">{tint.shopLabel ?? tint.label}</span></div>
    <div className="swatch-list" role="group" aria-label={label}>{acrylicTints.map(option => <button type="button" key={option.id} className={`tint-swatch ${option.id === tint.id ? "tint-swatch-active" : ""}`} style={{ "--swatch": option.color } as CSSProperties} aria-label={`${option.label} (${option.shopLabel})`} aria-pressed={option.id === tint.id} title={`${option.label} · ${option.shopLabel}`} onClick={() => onChange(option)}><span className={`swatch-surface ${option.id === "clear" ? "swatch-colorless" : ""}`}>{option.id === tint.id && <Check className="swatch-check" size={16} />}</span><span className="swatch-caption">{option.label}</span></button>)}</div>
  </>;
}
