...
export function PreviewStage({ config, dark }: { config: CaseConfiguration; dark: boolean }) {
  ...
  return <section className={`preview-stage ${expanded ? "preview-expanded" : ""}`} aria-label="Interactive 3D case preview" onKeyDown={event => { if (event.key === "Escape") setExpanded(false); }}>
    <div className="stage-topline"><div className="model-label"><span className="status-dot" /><span>LIVE PREVIEW</span><span className="model-label-separator">/</span><span>{config.rows * 3}U / {config.hp}HP</span></div><span className="stage-material">GS—{config.thickness.toString().padStart(2, "0")} <span>•</span> {config.tint.label.toUpperCase()}</span></div>
    <div className="stage-watermark" aria-hidden="true">A—508</div>
    <div className="canvas-wrap"><Suspense fallback={<div className="preview-fallback" role="status">Preparing your case…</div>}><CasePreview config={config} dark={dark} view={view} resetKey={resetKey} exploded={exploded} modules={modules} /></Suspense></div>
    <div className="stage-annotation"><span className="annotation-line" /><div><span>{config.thickness} MM CAST ACRYLIC</span><span>{config.tint.label} / polished edge</span></div></div>
    ...
    <div className="stage-caption"><span><Check size={12} />{exploded ? "Assembly study" : "Real-time material preview"}</span><span>CONCEPT MODEL · MM</span></div>
  </section>;
}
