  ...
  return <aside className="control-panel accordion-control-panel case-control-panel" aria-label="Case controls">
    ...
    <ConfigSection number="06" title="Power & assembly" summary={powerSummary}>
      ...
      {config.busboard === "compactpwr" && <>
        <div className="inline-field"><label htmlFor="compactpwr-inlet-side">Inlet panel</label><div className="select-wrap board-select"><select id="compactpwr-inlet-side" value={config.compactPwrInletSide ?? "left"} onChange={event => onChange({ compactPwrInletSide: event.target.value as "left" | "rear" })}><option value="left">Left side panel</option><option value="rear">Back panel</option></select><ChevronDown size={12} /></div></div>
        <p className="control-note board-note">CompactPWR · 174 × 79 × 20 mm · 20 headers. Centred on the base, rotated 180° with the power connector on the left, near the inlet. Four photo-estimated corner mounts (Ø3.2 mm assumed); verify the hole pattern and hardware before drilling.</p>
        <p className="control-note" role="status">Preview assumes 5 mm insulating standoffs. {panels.powerBoard?.fits ? `${panels.powerBoard.moduleClearance.toFixed(1)} mm remains above the board for modules and cables.` : "Board does not fit: allow at least 174 × 79 mm inside the case (35 HP and at least 2U total). Preview and bottom mounting holes are omitted."}</p>
        <p className="control-note" role="status">{panels.inlet?.fits ? `${panels.inlet.side === "rear" ? "Back-panel" : "Left-side"} barrel/switch inlet: 45 × 25 mm clearance window and two Ø3.5 mm screw holes, 59 mm apart, for the supplied 70 × 40 mm plate. The window is a design allowance; verify the assembled inlet’s rear clearance before cutting.` : `The 70 × 40 mm inlet plate does not fit clear of the ${config.compactPwrInletSide === "rear" ? "back" : "left"} panel’s joints and rails. Increase the case dimensions or choose the other panel; inlet cuts and preview are omitted.`} The 3D switch, socket and hardware are illustrative. USB-C inlet geometry and cable routing are not reserved.</p>
        {panels.mountingConflicts > 0 && <p className="control-note" role="alert">Bottom custom cutouts overlap or approach {panels.mountingConflicts} mounting points. Move these cuts to preserve the mounts.</p>}
      </>}
