  ...
  return <aside className="control-panel accordion-control-panel case-control-panel" aria-label="Case controls">
    ...
    <ConfigSection number="06" title="Power & assembly" summary={powerSummary}>
      ...
      {config.busboard === "compactpwr" && <>
        ...
        <p className="control-note" role="status">{panels.inlet?.fits ? "Left-side barrel/switch inlet: 45 × 25 mm clearance window and two Ø3.5 mm screw holes, 59 mm apart, for the supplied 70 × 40 mm plate. The window is a design allowance; verify the assembled inlet’s rear clearance before cutting." : "The 70 × 40 mm inlet plate does not fit clear of the left panel’s joints and rail holes. Increase the case depth or row size; inlet cuts are omitted."} USB-C inlet geometry and cable routing are not reserved.</p>
        {panels.mountingConflicts > 0 && <p className="control-note" role="alert">Bottom custom cutouts overlap or approach {panels.mountingConflicts} mounting points. Move these cuts to preserve the mounts.</p>}
      </>}
