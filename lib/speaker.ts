...
import { maxCutouts, subtractCutouts, type CustomCutout } from "./custom-cutouts";
...
export function normalizeSpeakerConfiguration(input: SpeakerConfiguration): SpeakerConfiguration {
  ...
  config.cutouts = (config.cutouts ?? []).filter(cutout => cutout.side === "rear").slice(0, maxCutouts);
  return config;
}
...
export function createSpeaker(input: SpeakerConfiguration) {
  ...
  part("rear-cover","Outer rear panel",w,h,bolts,[0,0,-d/2-config.grilleGap-t("rear-cover")/2],[0,Math.PI,0],[0,0,-80]);
  // Local X faces right when seen from behind, so text reads from outside.
  const rearCover = parts[parts.length-1];
  const rearResult = subtractCutouts(rearCover.polygons,config.cutouts,"rear");
  const cutoutPanels = { faces: { rear: { ...rearResult, original: rearCover.polygons } }, reports: [rearResult.report] };
  rearCover.polygons = rearResult.polygons;
  ...
  for (const panel of parts) {
    ...
    if (panel.id === "rear" || panel.id === "rear-cover") panel.position[2] -= gasket;
  }
  ...
  const rearZ = Math.min(-d/2-gasket-config.grilleGap-t("rear-cover"),config.handle ? handleCentreZ-config.handleWidth/2 : -d/2-gasket);
  ...
  return { config, parts, dampingParts, cutoutPanels, canExport: !rearResult.report.empty && !rearResult.report.error, mounts, driverMounts, panelMounts, carrierJoints, feet, handles, totalHeight:topY-floorY, floorY, topY, leftX, rightX, overallWidth:rightX-leftX, rearZ, frontZ, overallDepth:frontZ-rearZ, dots, innerWidth, innerHeight, innerDepth:innerDepth+2*gasket, grossVolumeLitres,
    bodyDepth: d+2*gasket, totalDepth: d+2*gasket+2*config.grilleGap+t("grille")+t("rear-cover"), openArea: dots.length*Math.PI*(config.dotDiameter/2)**2/((w-2*config.grilleBorder)*(h-2*config.grilleBorder))*100 };
}
...
export const speakerBuildNotes = [
  ...
  "Bond the top, base and side sheets into a sealed shell. Without optional damping, bond the baffle too; with damping, clamp it against the front gasket using the four M3 corner tie rods. The rods also retain the removable rear with threaded spacers and load-spreading washers. The preview includes these fasteners, driver screws and board standoffs. Mount the grille and outer rear panel on matching spacers at those same centres. Seal penetrations and retain service access.",
  ...
  "The dot grille and outer rear panel sit outside the acoustic chamber on matching standoffs. SVG or text cutouts pass through only the outer rear panel; its local coordinates and cutting layout face outward so lettering reads from behind. Loose islands are removed after all cuts. Keep the four corner mounts intact. Keep both passive radiators free to move. Gross internal volume excludes drivers, boards, battery and bracing; it is not the stock acoustic volume. Prototype sealing, panel resonance, radiator travel and DSP tuning. The replacement enclosure has no validated acoustic or IP rating.",
];
export const speakerHardware = ["MYND donor: 1 woofer, 2 tweeters, 2 passive radiators; retain original gaskets and frames", "Original MYND electronics, protected battery pack, controls, USB-C/AUX pods and wiring", "4 M3 corner tie rods, 8 threaded front/rear spacers, 8 outer-panel screws and 24 corner washers; size to the chosen depth", "20 illustrative M3 driver/radiator screws and washers; verify thread and engagement against the donor", "Front and rear damping frames when enabled; otherwise a donor-measured rear seal. Acrylic-compatible bonding system and port/control sealing gaskets", "19 PCB standoffs with board-side and sheet-side screws / washers; 8 HMI-cover supports with screws / washers on both sides", "Port housing: 2 M2.5 and 2 M3 through screws, 4 nuts, 8 washers and 2 counterbore sleeves (7 mm); verify donor fit", "Battery restraint and driver mounting adapters / retainers"];
...
export function speakerExport(speaker: Speaker) {
  return { product:"Acryl508",mode:"speaker",version:1,units:"mm",status:"unvalidated-prototype",configuration:speaker.config,
    ...
    rearCover:{gapMm:speaker.config.grilleGap,cutouts:speaker.config.cutouts,reports:speaker.cutoutPanels.reports},canExport:speaker.canExport,
...
export function speakerSvg(speaker: Speaker) {
  if (!speaker.canExport) throw new Error("Resolve the rear-panel cutout warnings before exporting cutting sheets.");
