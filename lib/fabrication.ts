...
export type FabricationPart = { id: string; label: string; polygons: MultiPolygon; engraving?: MultiPolygon; material: string; thickness?: number };
...
export function artFabrication(art: Art): Fabrication {
  return { parts: art.parts.map(part => ({ id: part.id, label: part.label, polygons: down(part.polygons), thickness: part.thickness, material: materialLabel(sheetMaterial(art.config, part.id).tint, sheetMaterial(art.config, part.id).transparency) })),
    ...
    hardware: ["No screws or adhesive. Assemble B slots-up and A slots-down, then form the leaves and insert leaf shelves level into their matching slots."], thickness: art.config.thickness, clearance: art.config.clearance, blocked: false };
}
...
export function packSheets(parts: FabricationPart[], width: number, height: number, gap: number, rotate: boolean) {
  ...
  for (const { part, bounds } of ordered) {
    ...
    for (const state of states.filter(state => state.sheet.material === part.material && state.sheet.thickness === part.thickness)) {
    ...
    if (!placement) {
      const sheet: StockSheet = { material: part.material, thickness: part.thickness, parts: [] }, state = { sheet, x: gap, y: gap, row: 0 };
...
export function stockSvg(sheet: StockSheet, width: number, height: number, warnings: string[] = []) {
  return `<?xml version="1.0" encoding="UTF-8"?>\n<svg xmlns="http://www.w3.org/2000/svg" width="${width}mm" height="${height}mm" viewBox="0 0 ${width} ${height}"><title>Acryl508 stock sheet · ${xml(sheet.material)}${sheet.thickness ? ` · ${sheet.thickness} mm` : ""}</title><desc>Millimetres. Red: cut. Blue: engrave. Apply kerf once in CAM. ${xml(warnings.join(" "))}</desc><g id="cut" fill="none" stroke="#ef4444" stroke-width="0.2">${sheet.parts.map((part, index) => `<g id="part-${index + 1}" data-part="${xml(part.id)}"${part.thickness ? ` data-thickness-mm="${part.thickness}"` : ""}><title>${xml(part.label)}</title><path d="${casePathData(part.polygons)}" /></g>`).join("")}</g><g id="engrave" fill="#2563eb" fill-rule="evenodd">${sheet.parts.filter(part => part.engraving?.length).map(part => `<path d="${casePathData(part.engraving!)}" />`).join("")}</g></svg>`;
}
