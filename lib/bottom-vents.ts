...
import type { VentCoverage, VentDensity, VentLayout, VentMix, VentStyle } from "./configurator";
...
export function ventOpeningPath(opening: VentOpening) {
  const { x, y, width, height, shape: style } = opening;
  const path = new Path();
...
export function createBottomVentLayout(width: number, innerLength: number, style: VentStyle = "long-slits", density: VentDensity = "medium", options: VentOptions = {}) {
  ...
  const mixed = style === "mixed", staggered = options.layout === "staggered", fullField = options.coverage === "field";
  ...
  const regularRowPitch = nominalHeight + Math.max(minimumWeb, requestedPitch - nominalWidth);
  // Round/hex holes use triangular spacing when staggered. Keep rectangular
  ...
  for (const side of [-1, 1]) for (let row = 0; row < rows; row++) {
    const globalRow = (side === -1 ? 0 : rows) + row;
    const rowColumns = staggered && globalRow % 2 === 1 ? Math.max(0, columns - 1) : columns;
    const cellWidth = Math.min(pitch - minimumWeb, usableWidth - (rowColumns - 1) * pitch);
    for (let column = 0; column < rowColumns; column++) {
      const alternate = options.mix === "rows" ? globalRow : options.mix === "columns" ? column : globalRow + column;
      const shape = mixed ? alternate % 2 === 0 ? "round" : "short-slits" : style;
      const isSlit = shape === "long-slits" || shape === "short-slits";
      const centerX = (column - (rowColumns - 1) / 2) * pitch;
      ...
      const openingWidth = isSlit ? 0.034 : minimumOpening + (nominalWidth - minimumOpening) * size;
      const openingHeight = isSlit ? minimumOpening + (maxHeight - minimumOpening) * size : openingWidth;
  ...
  return { paths: openings.map(ventOpeningPath), openings, minimumWeb, edgeMargin, minimumOpening, limited, omitted, pitchAdjusted: pitch > requestedPitch };
}
